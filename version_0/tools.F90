...
module de_tools
    ...
    subroutine write_vasp(tag)
        ...
        if(altermagnetic) then
            ...
            do i = 1, pstruct(tag)%natom
                if(is_magnetic_atom(pstruct(tag)%ptype(i))) then
                    pstruct(tag)%SelectiveDynamics(1:3, i) = .false.
                else
                    pstruct(tag)%SelectiveDynamics(1:3, i) = .true.
                end if
            end do
        end if
    ...
    subroutine write_dmol3(tag)
        use kinds
        use parameters, only : pstruct, sys_name, name_element
        use parameters, only : DMOL3_PBC, fix_a, fix_b, fix_c, fix_alpha, fix_beta, fix_gama
        use init_struct, only : dir2car
        implicit none
        integer(i4b), intent(in)  :: tag
        character(len=20) :: fname
        character(len=20) :: line_starter
        integer(i4b) :: i,j,k,kk
        real(dp) :: dir(3), car(3), lattice(3,3)
        fname = "cluster.car"//trim(itoa(tag))
        open(4113, file=fname)
        write(4113, "(A17)")"!BIOSYM archive 3"
        if(DMOL3_PBC) then
            write(4113, "(A6)")"PBC=ON"
        else
            write(4113, "(A7)")"PBC=OFF"
        end if
        write(4113, *)""
        write(4113, "(A34)")"!DATE     start struct from IM2ODE"
        if(DMOL3_PBC) then
           write(4113, "(A3,6F10.4)")"PBC",fix_a, fix_b, fix_c, fix_gama, fix_beta, fix_alpha
        end if
        i = 0
        do k = 1, pstruct(tag)%ntyp
            do kk = 1, pstruct(tag)%nelement(k)
                i = i + 1
                dir = pstruct(tag)%pos(:, i)
                lattice = pstruct(tag) % lat
                call dir2car(lattice, dir, car)
                line_starter=trim(name_element(k))//trim(itoa(kk))
                write(4113, "(A5,F15.9,F15.9,F15.9,A21,A2,A7)") &
                    line_starter,car(1),car(2),car(3)," XXXX 1      xx      ",name_element(k),"  0.000"
            end do
        end do
        write(4113,"(A3)") "end"
        write(4113,"(A3)") "end"
        close(4113)
    end subroutine write_dmol3 
    ...
    subroutine read_dmol3(tag)
        use kinds
        use parameters, only : pstruct
        use parameters, only : DMOL3_PBC
        use constants, only : max_atom, max_type
        use init_struct, only : car2dir
        implicit none
        integer(i4b), intent(in) :: tag
        integer(i4b) :: i, kk, lth, f1, endn
        integer(i4b) :: natom
        real(dp) :: inf = 1e5
        ...
        real(dp) :: energy, tmp
        logical :: alive
        character(len=10) :: string(10)
        character(len=200) :: strin
        character(len=40) ::  nametag, number
        call system("cd dmol3_"//trim(itoa(tag))//"; cp cluster.car OUTCAR ..;")
        inquire(file = "cluster.car", exist = alive)
        if(alive) then
            open(4321, file = "cluster.car", status = "old")
        else
            write(1224, *) "no cluster.car of pop ",  tag
            pstruct(tag) % energy = inf
            return
        end if
        endn = 4
        ...
        do i = 1, natom
          read(4321, *, iostat = f1) string(1), car(1), car(2), car(3), &
              string(2), kk, string(3), string(4), tmp
          if(f1 /= 0) then
              pstruct(tag) % energy = inf
              close(4321)
              return
          end if
          call car2dir(lat_matrix, car, dir)
          pstruct(tag) % pos(1, i) = dir(1) - floor(dir(1))
          pstruct(tag) % pos(2, i) = dir(2) - floor(dir(2))
          pstruct(tag) % pos(3, i) = dir(3) - floor(dir(3))
        end do
        close(4321)
        
        inquire(file = "OUTCAR", exist = alive)
        if(alive) then
            open(4322, file="OUTCAR", status = "old")
            energy = inf
        else
            write(1224, *) "no OUTCAR of pop ", tag
            pstruct(tag) % energy = inf
        ...
        do while(.true.)
            read(4322, fmt="(A200)", iostat=f1) strin
            if(f1 /= 0) exit
            lth = index(strin, '=')
            if(lth /= 0 .and. lth /= 1) then
                read(strin(:lth-1), "(A40)") nametag
