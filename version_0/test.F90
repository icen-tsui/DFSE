...
use de_tools, only: read_dmol3, write_dmol3
...
!use run_lammps
use differencial_evolution, only : run_de_vasp, run_mode_vasp, run_de_vasp_symmetric
use sort, only : sort_results_mode
use parameters, only : Pickup, Pickup_step, Mystruct
use parameters, only : PWMAT, DMOL3
...
do j = step, max_step
    ...
    if(.not. Pickup) then
        ...
        write(*, *)"run pvasp"
        write(1224, *) "run pvasp"
        flush(1224)
        if(PWMAT) then
            call system("bash run_pwmat.sh")
        else if(DMOL3) then
            call system("bash run_dmol3.sh")
        else
    ...
    write(*, *) "de operation"
    if (mode) then
        print *, ">>> Running MODE_VASP branch"
        call run_mode_vasp(j)

    else if (polarization_2d .or. polarization_3d) then
        print *, ">>> Running SYMMETRIC DE branch (polarization mode)"
        call run_de_vasp_symmetric(j)

    else
        print *, ">>> Running NORMAL DE branch"
        call run_de_vasp(j)
    end if
