...
module init_mod
    ...
    subroutine read_input()
        ...
        use parameters, only : PWMAT, atm_num_element, DMOL3, DMOL3_PBC
        ...
        use parameters, only : polarization_3d,crystal_system_3d,crystal_system_count_3d
        use parameters, only : polarization_2d,crystal_system_2d,crystal_system_count_2d
        use parameters, only : altermagnetic, magnetic_types, non_magnetic_types
        use parameters, only : num_magnetic_types, num_non_magnetic_types
        use constants, only : max_type
        use kinds
        implicit none

        integer(i4b) :: n, i, j, k, line, f1, lth
        integer(i4b) :: ios, pos_cur, val
        character(len=40) :: nametag(200), number(200)
        character(len=200) :: strin(200), strtmp, listbuf
        logical :: flag, mag_hit
        ...
        altermagnetic = .false.
        magnetic_types = 0
        non_magnetic_types = 0
        num_magnetic_types = 0
        num_non_magnetic_types = 0
        call find(nametag, "Altermagnetic", i)
        if(i /= 0) then
            read(number(i), *) altermagnetic
        end if
        if(altermagnetic) then
            call find(nametag, "MagneticSpecies", j)
            if(j /= 0) then
                listbuf = adjustl(trim(number(j)))
                magnetic_types = 0
                num_magnetic_types = 0
                read(listbuf, *, iostat=ios) (magnetic_types(k), k = 1, max_type)
                do k = 1, max_type
                    if(magnetic_types(k) > 0) num_magnetic_types = num_magnetic_types + 1
                end do
            end if

            call find(nametag, "NonMagneticSpecies", j)
            if(j /= 0) then
                listbuf = adjustl(trim(number(j)))
                non_magnetic_types = 0
                num_non_magnetic_types = 0
                read(listbuf, *, iostat=ios) (non_magnetic_types(k), k = 1, max_type)
                do k = 1, max_type
                    if(non_magnetic_types(k) > 0) num_non_magnetic_types = num_non_magnetic_types + 1
                end do
            end if

            if(num_non_magnetic_types == 0) then
                do k = 1, num_species
                    mag_hit = .false.
                    do j = 1, num_magnetic_types
                        if(magnetic_types(j) == k) mag_hit = .true.
                    end do
                    if(.not. mag_hit) then
                        num_non_magnetic_types = num_non_magnetic_types + 1
                        non_magnetic_types(num_non_magnetic_types) = k
                    end if
                end do
            end if
        end if
        ...
        call find(nametag, "DMOL3", i)
        if(i == 0) then
            write(1224, *) "USE VASP"
            DMOL3 = .false.
        else
            read(number(i), *) DMOL3
            call find(nametag, "DMOL3_PBC", i)
            if(i == 0) then
                write(1224, *) "Input DMOL3_PBC"
                DMOL3_PBC = .false.
            else
                read(number(i), *) DMOL3_PBC
            end if
        end if 
        ...
        call find(nametag,"polarization_3d",i)
        if (i==0) then
            polarization_3d = .false.
        else
            read(number(i),*) polarization_3d
            write(1224,*) "Input polarization_3d"

            call find(nametag,"crystal_system_count_3d",i)
            if (i==0) then
                crystal_system_count_3d = 7
            else
                read(number(i), *) crystal_system_count_3d
            end if

            call find(nametag,"crystal_system_3d",i)
            if (i==0) then
                crystal_system_3d = [1,2,3,4,5,6,7]
                write(1224,*) crystal_system_3d
            else
                allocate(crystal_system_3d(crystal_system_count_3d))
                read(number(i), *) crystal_system_3d
                write(1224,*) "read crystal!", crystal_system_3d
            end if
        end if

        call find(nametag,"polarization_2d",i)
        if (i==0) then
            polarization_2d = .false.
        else
            read(number(i),*) polarization_2d
            write(1224,*) "Input polarization_2d"

            call find(nametag,"crystal_system_count_2d",i)
            if (i==0) then
                crystal_system_count_2d = 6
            else
                read(number(i), *) crystal_system_count_2d
            end if

            call find(nametag,"crystal_system_2d",i)
            if (i==0) then
                crystal_system_2d = [1,2,3,4,5,6]
                write(1224,*) crystal_system_2d
            else
                allocate(crystal_system_2d(crystal_system_count_2d))
                read(number(i), *) crystal_system_2d
                write(1224,*) "read crystal!", crystal_system_2d
            end if
        end if
    ...
    subroutine write_input()
        ...
        write(1224, *) "Altermagnetic: ", altermagnetic
        if(altermagnetic) then
            write(1224, *) "MagneticSpecies: ", (magnetic_types(i), i = 1, num_magnetic_types)
            write(1224, *) "NonMagneticSpecies: ", (non_magnetic_types(i), i = 1, num_non_magnetic_types)
        end if
        write(1224, *) "PWMAT: ", PWMAT
        write(1224, *) "DMOL3: ", DMOL3
        write(1224, *) "DMOL3_PBC: ", DMOL3_PBC
        ...
        if(polarization_3d) then
           write(1224, *) "polarization_3d: ", polarization_3d
           write(1224, *) "crystal_system_3d: ", crystal_system_3d
           write(1224, *) "crystal_system_count_3d: ",crystal_system_count_3d
        end if
        if(polarization_2d) then
           write(1224, *) "polarization_2d: ", polarization_2d
           write(1224, *) "crystal_system_2d: ", crystal_system_2d
           write(1224, *) "crystal_system_count_2d: ",crystal_system_count_2d
        end if
